...
export default function TVMode() {
    ...
    fetch('/api/videos')
      ...
      .then(data => {
        if (data.videos?.length > 0) setVideos(data.videos);
          ...
          { id: '1', title: 'Sunday Service', category: 'Sermons', youtube_url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', featured: true } as Video,
        ]);
      });
  }, []);
  ...
  const handleUnmute = () => {
    setIsMuted(false);
    setShowUnmute(false);
    setIframeKey(prev => prev + 1); // Reload iframe with unmuted
  };
        ...
        {currentVideo?.youtube_url ? (
          ...
          <div className="text-center">
            <div className="w-32 h-32 mx-auto mb-8 rounded-full bg-blue-600/20 flex items-center justify-center">
              <svg className="w-16 h-16 text-blue-400" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
            </div>
            <p className="text-white/60 text-xl">No videos</p>
            <a href="/" className="mt-4 inline-block text-blue-400 hover:underline">Go to Home</a>
          </div>
        )}
      </div>
